...
collapse.analysis=function(z)
{
    ...
    if ( !("is.permu" %in% names(z)) )
    {
       if (ytype=="Q") ytp="C"
       if (ytype=="B") ytp="D"
       if (!is.null(covars))  try(call.skat(Z=as.matrix(data[,xnames]),Y=data[,trait],ytype=ytp,X=data[,covars,drop=FALSE])) -> fit
       if (is.null(covars))  try(call.skat(Z=as.matrix(data[,xnames]),Y=data[,trait],ytype=ytp,X=NULL)) -> fit
    ...
    call.VT.price=function(Z,Y,X=NULL)
    {
        ...
        if (!is.null(X)) {
            md="Y"; for (i in colnames(X)) md=paste(md,i,sep="+")
            sub("[+]","~",md)->md
            lm(formula(md),data=cbind(X,Y))$resid->Y
        }
    ...
    ### call VT
    if (!is.null(covars))  try(call.VT.price(Z=as.matrix(data[,xnames]),Y=data[,trait],X=data[,covars,drop=FALSE])) -> fit
    ...
    w=1;colSums(x*w,na.rm=T)->xi; cast=xi # CAST
    ...
    ms=list(
        SUM=list(var="cast",test="glm",alt="two.sided",id=!is.na(y)),
        C01=list(var="cmc",test="glm",alt="two.sided",id=!is.na(y)), 
        xdw=list(var="xdw",test="glm",alt="two.sided",id=!is.na(y)),
        xup=list(var="xup",test="glm",alt="two.sided",id=!is.na(y)),
        PWST=list(var="pwst",test="glm",alt="two.sided",id=!is.na(y)), 
        ...
        WSS=list(var="wss",test="glm",alt="two.sided",id=!is.na(y))
    )
    ...
    for (i in names(ms))
    {
        ...
        if (vi %in% colnames(xc)) {
            ...
            #if (class(fit)=="try-error")  #save(z,xc,ids,vi,file="z")
...
collapse.test=function(z,permu=0)
{
	...
	z$is.permu=T
        ...
        rst$pp=pp/permu
        #rst$ppp=ppp  we don't need this detailed output anymore 
